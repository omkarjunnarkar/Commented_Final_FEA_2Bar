...
Objective:	 To perform Numerical simulation of a 1-D Truss problem, with 2 Bars set along one axis with an Axial Force acting on the center and fixed on two ends. [ Refer Report/Manual ]
Input:		 Open 'UserInputFile2BarFEA.csv' and Input the values mentioned in the file. Details/Symbols of the Input values/parameters provided below in the code.
...
Steps:	1. Make sure the environment is set for C++17 (Necessary for GNUPLOT Interface)
		...
		9. Edit the 'UserInputFile2Bar.csv' and run main.cpp.
...
Length_1 : Length of left element(mm)
Length_2 : Length of right element(mm)
...
displacement_2_bar.csv : Displacement of the Selected/Focus/Center Node at each load step
stress.csv : Stress of Selected/Focus Element at each load step
strain.csv : Strain of Selected/Focus Element at each load step
...
InputFileFEA2Bar.txt : Input Database File
...
void main() {
	...
	*/
	...
	double Length_1 = user_input[3];				
	double Length_2 = user_input[4];				
	double E = user_input[5]*1e3;		/* Multiplied by 1e3 to convert from GPa to MPa */
	int steps_num = 10000;				/* If needed, can be changed here */
	double steps = steps_num;
	double t_tot = user_input[6];				
	double yield = user_input[7];			
	double eta = user_input[8];					
	int elements = user_input[9];			
	...
	*/
	...
	while (abs(force) <= abs(ForceMax) ) {
		...
		*/
		...
		force = force + (1 / double(steps_num)) * double(ForceMax);
	}
